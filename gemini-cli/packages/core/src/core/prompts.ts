...
export function getCoreSystemPrompt(userMemory?: string): string {
  ...
  // Check if we're using Llama.cpp (local model) for minimal prompt
  const isLlamaMode = process.env.GEMINI_MODE === 'llama' || 
                      process.env.LLAMA_CPP_ENDPOINT || 
                      process.env.GEMINI_API_ENDPOINT?.includes('localhost') ||
                      process.env.GEMINI_API_ENDPOINT?.includes('127.0.0.1') ||
                      true; // Force minimal prompt for now to test
